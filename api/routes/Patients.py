...
def jsonToPandasToCsv(jsonInput):
    ...
    for jsons in jsonInput:
        for item in jsons:
            ...
            try:
                row[12] = item['resource']['managingOrganization']['display']  # Managing Organization Display Name
            except Exception as e:
                print(e)
            try:
                row[13] = item['resource']['managingOrganization']['identifier'][
                        'value']  # Managing Organisation Identifier Value
            except Exception as e:
                print(e)
    ...
    df = pd.DataFrame(outputArray, columns=["givenName", "familyName", "gender", "birthDate", "phoneNumber", "city",
                                            "country", "postalCode", "county", "parish", "village", "nationalID","facilityName","facilityIdentifier"])
